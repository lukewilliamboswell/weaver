...
RELEASE_PACKAGE_PREFIXES = (
    "https://github.com/lukewilliamboswell/weaver/releases/download/",
)
...
def is_weaver_package_dependency(dependency: str) -> bool:
    return dependency == LOCAL_PACKAGE_PATH or any(
        dependency.startswith(prefix) and dependency.endswith(".tar.zst")
...
def copy_examples_with_bundle_url(examples_dir: Path, bundle_url: str) -> list[Path]:
    ...
    for example in examples:
        ...
        for match in PACKAGE_DEPENDENCY_RE.finditer(source):
            ...
            if is_weaver_package_dependency(dependency):
                rewritten = (
                    source[: match.start()]
        ...
        if rewritten is None:
            raise SystemExit(f"{example.name} does not use the expected weaver package dependency")
...
def main() -> None:
    ...
    default_tmp = ROOT / ".weaver-tmp"
    tmp_parent = Path(os.environ.get("WEAVER_TMPDIR", default_tmp))
    tmp_parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="weaver-bundle-", dir=tmp_parent) as tmp:
        tmp_dir = Path(tmp)
